/**
 * EPO API Sync Route: Internal Qualifications
 * Endpoint: /api/epo-sync-internal
 * Syncs internal qualifications data to EPO API
 */

import { NextRequest, NextResponse } from 'next/server';
import { EPO_API_CONFIG, EpoApiResponse } from '@/lib/api/epo-api-types';

interface InternalQualificationRecord {
  mesec_from?: number;
  godina_from: number;
  mesec_to?: number;
  godina_to?: number;
...
interface RequestBody {
  ...
  epoUserId: string;
  records: InternalQualificationRecord[];
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🟣 [EPO Sync Internal] Starting sync...');
    ...
    if (!records || records.length === 0) {
      return NextResponse.json(
        { error: 'No internal qualifications to sync' },
        { status: 400 }
      );
    }
    ...
    console.log('🟣 [EPO Sync Internal] Complete!');
    ...
    console.error('🟣 [EPO Sync Internal] Fatal error:', error);
    return NextResponse.json(
      {
        error: 'Failed to sync internal qualifications',
